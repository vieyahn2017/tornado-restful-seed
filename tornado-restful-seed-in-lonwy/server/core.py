...
# Last modified:2017-06-06 14:58:31
# Filename:core.py
# Description:
import copy
import sys
...
class Session(object):
    ...
    def build_session(self):
        ...
        cache_client().set_multi(submit_data, settings["token_duration"])
    ...
    def get_session(handler):
        ...
        if token_value:
            token_user_id = "%s.userid" % token_value
            query_id = cache_client().get(token_user_id)
            if query_id:
                cached_user = cache_client().get(query_id)
                if not cached_user:
...
class Application(object):
    ...
    def start(self):
        ...
        ioloop.IOLoop.current().set_blocking_log_threshold(0.5)
